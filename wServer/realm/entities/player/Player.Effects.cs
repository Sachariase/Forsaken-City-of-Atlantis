...
namespace wServer.realm.entities.player
{
    partial class Player
    {
        ...
        private void HandleEffects(RealmTime time)
        {
            ...
            if (HasConditionEffect(ConditionEffectIndex.PetStasis) &&
                PetStasis > 0)
            {
                if (PetStasis > 0)
                {
                ...
                float pet = 0;
                pet += 28 * (time.thisTickTimes / 1000f);
            }
        ...
        private bool CanPetStasis()
        {
            if (HasConditionEffect(ConditionEffectIndex.PetStasis) || ninjaShoot)
                return false;
            return true;
        }
